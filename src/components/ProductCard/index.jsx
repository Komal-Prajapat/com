...
const 	ProductCard = ({ renderproduct, shownav, setProductList ,productList , reload , setReload}) => {
	...
	return (
		<>
			<div className={`product_grid ${shownav ? "productflex " : "grid_container_product"} `}>
							...
							<div className={`block2 ${shownav ? "pro_container" : ""} `}>
								<div className="block2-pic hov-img0 Product_container">
									<div className="block2-txt-child2 flex-r p-t-3 like_icon-div">
										{product.is_wishlist ?
											<FavoriteIcon sx={{ color: "var(--colorprimary)", cursor: "pointer" }} onClick={() => handlelikeremove(product.id ? product.id : product.productId, index, "remove", user_id)} />
											:
											<FavoriteBorderIcon sx={{ cursor: "pointer" }} onClick={() => handlelikeremove(product.id ? product.id : product.productId, index, "add", user_id)} />}
									</div>

									<div className='image_container_product'>
										<div class="product-img" onClick={() => handleDetailPage(product.productId ? product.productId : product.id)}>
											<img src={`${ImageUrl}${product.file ? product.file : product.files}`} alt="front product img" class="main-img" />
											<img src={`${ImageUrl}${product?.secondary_image}`} alt="" class="rear-img" />
										</div>
									</div>
								</div>
								...
								<div className="block2-txt flex-w flex-t p-t-5">
									<div className="block2-txt-child1 flex-col-l ">
										<p className="stext-104 hov-cl1 trans-04 js-name-b2 C_pointer product_card_product_name">
											{product.productName ? product.productName : product.product_name}
										</p>

										<span className="stext-105 text_name">
											<CurrencyRupeeIcon sx={{ fontSize: "16px" }} />{product.mrp_amount}/-
										</span>
									</div>
								</div>
							</div>
			...
			{showDetails ? <SimpleDialog showDetails={showDetails} onClick={closeModel} /> : ""}
