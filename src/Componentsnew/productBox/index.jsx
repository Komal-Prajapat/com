        ...
        if (userId) {
            try {
                if (type === "remove") {
                    ...
                    const updatedList = renderproduct.filter(item => item.productId !== id);
                    ...
                    <div className='product_box' key={index}>
                                ...
                                <ul>
                                    <li>
                                        <ShoppingBagOutlinedIcon
                                            ...
                                            sx={{ cursor: item.is_active === 0 ? "no-drop" : "pointer" }}
                                            onClick={() => item.is_active === 0 ? "" : handleDetailPage(item.productId || item.id, item.product_name || item.productName)}
                                        />
                                    </li>
                                    <li>
                                        {item.is_wishlist ?
                        ...
                        {item.is_active === 0 ? <p className='outofstock test-align'>Out Of Stock</p> : ""}
                            <p className='product_name_con'>
                                <p className='product_name' onClick={() => handleDetailPage(item.productId || item.id, item.product_name || item.productName)}>
                            ...
                            <p className='product_price'>
                                ...
                                {item.discount_percent === 0 ?
                                    <span className='mrpwithdiscount'><CurrencyRupeeIcon /> {item.discount_amount || item.price}</span>
                                    :
                                    <>
                                        <span className='mrpwithdiscount'><CurrencyRupeeIcon  />{item.discount_amount}</span>
                                        <strike className='discount_mrp'>{item.mrp_amount || item.price}</strike>
                                        <span className='discount_percent'>{item.discount_percent}% off </span>
                                    </>
                                }
                            </p>
